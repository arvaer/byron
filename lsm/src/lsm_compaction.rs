...
use crate::level::{Level, LevelMutex};
...
impl LsmDatabase {
    async fn extend(&mut self, from: usize) -> Result<(), LsmError> {
        ...
        let reference_level = self.levels.last().unwrap().read().await;
        let new_width = (reference_level.width as f64 * self.capacity_expansion_factor) as usize;
        let new_level = Level {
            inner: Vec::new(),
            depth: from + 1,
            width: new_width,
        ...
        self.levels.push(Arc::new(LevelMutex::new(new_level)));
        Ok(())
    }
    ...
    pub async fn insert_new_table(
        ...
        if level_number >= self.levels.len() {
            ...
            for (i, level_mutex) in self.levels.iter().enumerate() {
        ...
        let level_mutex = self.levels[level_number].clone();
        level_mutex
            .update_inner(|level| {
        ...
        let level_state = level_mutex.read().await;
        let needs_compaction = level_state.inner.len() >= level_state.width;
        drop(level_state);

        if needs_compaction {
            println!("Step 4: Level {} needs compaction", level_number);
            let compaction_task = crate::wall_e::CompactionTask::CompactLevel {
                level: {
                    let level = level_mutex.read().await;
                    level.clone()
                },
            ...
            *self.pending_compactions.entry(level_number).or_insert(0) += 1;
            ...
            println!(
                "Step 4.1: Notifying that level {} is ready for compaction",
                level_number
            );
...
mod tests {
    ...
    async fn test_extend() {
        ...
        assert_eq!(db.levels[1].get_width().await, 3); // 2 * 1.5 = 3
        assert_eq!(db.levels[1].get_depth().await, 1);
        assert_eq!(db.levels[1].get_len().await, 0);

        // Test extending from level 1
        db.extend(1).await.unwrap();
        assert_eq!(db.levels.len(), 3);
        assert_eq!(db.levels[2].get_width().await, 4); // 3 * 1.5 = 4.5, rounded to 4
        assert_eq!(db.levels[2].get_depth().await, 2);
    }
    ...
    async fn test_insert_without_compaction() {
        ...
        assert_eq!(db.levels[0].get_len().await, 1);
        assert_eq!(db.levels[0].get_entries().await, 10);
    }
    ...
    async fn test_insert_with_compaction() {
        ...
        assert_eq!(db.levels.len(), 2);
        assert_eq!(db.levels[1].get_len().await, 1);
        assert_eq!(db.levels[1].get_entries().await, 25); // 10 + 15
    }
    ...
    async fn test_multi_level_compaction() {
        ...
        assert_eq!(db.levels[1].get_len().await, 2);
        ...
        assert_eq!(db.levels[1].get_len().await, 0);
        assert_eq!(db.levels[2].get_len().await, 1);
        assert_eq!(db.levels[2].get_entries().await, 50); // 5 tables * 10 entries
    }
}
